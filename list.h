...
#define list_insert(TYPE, p_l, p_node_node, item) list_func(insert, TYPE)(p_l, p_node_node, item)
#define list_erase(TYPE, p_l, p_node_node) list_func(erase, TYPE)(p_l, p_node_node)
...
#define list_imp_h(TYPE) \
	...
	node(TYPE)* list_func(push_front, TYPE)(list(TYPE)* p_l, TYPE item); \
    node(TYPE)* list_func(push_back, TYPE)(list(TYPE)* p_l, TYPE item); \
    bool list_func(pop_back, TYPE)(list(TYPE)* p_l); \
    bool list_func(pop_front, TYPE)(list(TYPE)* p_l); \
	node(TYPE)* list_func(insert_after_node, TYPE)(list(TYPE)* p_l, node(TYPE)* p_target, TYPE item); \
    node(TYPE)* list_func(insert_before_node, TYPE)(list(TYPE)* p_l, node(TYPE)* p_target, TYPE item); \
    bool list_func(remove_node, TYPE)(list(TYPE)* p_l, node(TYPE)* p_node); \
    node(TYPE)* list_func(insert, TYPE)(list(TYPE)* p_l, size_t index, TYPE item); \
    bool list_func(remove, TYPE)(list(TYPE)* p_l, size_t index); \
...
#define list_imp_c(TYPE) \
    ...
    \
	node(TYPE)* list_func(insert_after_node, TYPE)(list(TYPE)* p_l, node(TYPE)* p_target, TYPE item) { \
        if (!p_target) return list_func(push_back, TYPE)(p_l, item); \
        node(TYPE)* p_new_node = (node(TYPE)*) malloc(sizeof(node(TYPE))); \
        if (!p_new_node) return NULL; \
        p_new_node->data = item; \
        p_new_node->p_prev = p_target; \
        p_new_node->p_next = p_target->p_next; \
        if (p_target->p_next) p_target->p_next->p_prev = p_new_node; \
        else p_l->p_back = p_new_node; \
        p_target->p_next = p_new_node; \
        p_l->size++; \
        return p_new_node; \
    } \
    \
    node(TYPE)* list_func(insert_before_node, TYPE)(list(TYPE)* p_l, node(TYPE)* p_target, TYPE item) { \
        if (!p_target) return list_func(push_front, TYPE)(p_l, item); \
        node(TYPE)* p_new_node = (node(TYPE)*) malloc(sizeof(node(TYPE))); \
        if (!p_new_node) return NULL; \
        p_new_node->data = item; \
        p_new_node->p_next = p_target; \
        p_new_node->p_prev = p_target->p_prev; \
        if (p_target->p_prev) p_target->p_prev->p_next = p_new_node; \
        else p_l->p_front = p_new_node; \
        p_target->p_prev = p_new_node; \
        p_l->size++; \
        return p_new_node; \
    } \
    \
    bool list_func(remove_node, TYPE)(list(TYPE)* p_l, node(TYPE)* p_node) { \
        if (!p_node || p_l->size == 0) return false; \
        if (p_node == p_l->p_front) return list_func(pop_front, TYPE)(p_l); \
        if (p_node == p_l->p_back) return list_func(pop_back, TYPE)(p_l); \
	...
	node(TYPE)* list_func(insert, TYPE)(list(TYPE)* p_l, size_t index, TYPE item) { \
        if (index == 0) return list_func(push_front, TYPE)(p_l, item); \
        if (index >= p_l->size) return list_func(push_back, TYPE)(p_l, item); \
        node(TYPE)* p_target = list_func(node_at, TYPE)(p_l, index); \
        return list_func(insert_before_node, TYPE)(p_l, p_target, item); \
    } \
    \
    bool list_func(remove, TYPE)(list(TYPE)* p_l, size_t index) { \
        if (index >= p_l->size) return false; \
        if (index == 0) return list_func(pop_front, TYPE)(p_l); \
        if (index == p_l->size - 1) return list_func(pop_back, TYPE)(p_l); \
        node(TYPE)* p_node = list_func(node_at, TYPE)(p_l, index); \
        return list_func(remove_node, TYPE)(p_l, p_node); \
    } \
    \
    TYPE* list_func(at, TYPE)(list(TYPE)* p_l, size_t index) { \
